...
Route::middleware('auth:sanctum')->prefix('admin/logs')->group(function () {
    ...
    Route::get('/view/{file}', function (string $file, Request $request) {
        ...
        $bytes = max(1024, min((int) $request->query('bytes', 65536), 2 * 1024 * 1024));
        ...
        if ($request->boolean('raw')) return response($content, 200, ['Content-Type' => 'text/plain; charset=UTF-8']);
    ...
    Route::post('/write-test', function (Request $request) {
        try {
            ...
            Log::error('[LOGS][WRITE-TEST][500] '.$e->getMessage(), ['file' => $e->getFile(), 'line' => $e->getLine()]);
            return response()->json(['ok' => false, 'error' => 'server_error'], 500);
        }
...
Route::get('/master/sekolah/cities', [MasterController::class, 'sekolahCities']); // NEW (opsional; selalu 200)
...
Route::middleware(['auth:sanctum'])->prefix('admin')->group(function () {
    ...
    Route::patch('/registrations/{id}', [RegistrationAdminController::class, 'update']);
    Route::delete('/registrations/{id}', [RegistrationAdminController::class, 'destroy']);
});

/**
 * OPTIONS preflight handler (CORS safety)
 */
